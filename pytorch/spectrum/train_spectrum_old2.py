...
class AddBlock(nn.Module):
    ...
    def reset_parameters(self):
        nn.init.kaiming_normal_(self.fc.weight, mode='fan_in', nonlinearity='selu')
...
class SinCosEmbedding(nn.Module):
    def __init__(self, width: int):
        ...
        k = torch.arange(1, width // 2 + 1).view(1, -1) # [1, W/2]
        self.register_buffer("omega", math.pi * k) # 2pi * [1,2,...,W/2]
...
class RGBEncoder(nn.Module):
    def __init__(self, in_dim: int = 3, width: int = 4, depth: int = 4, out_dim: int = 4):
        super().__init__()
        self.sin_width = (((width // in_dim) // 2) * 2)
        print(f"self.sin_width: {self.sin_width}")
        self.pre_proj = SinCosEmbedding(self.sin_width)
        
        self.in_proj = nn.Linear(self.sin_width * in_dim, width)
    ...
    def reset_parameters(self):
        nn.init.kaiming_normal_(self.in_proj.weight, mode='fan_in', nonlinearity='selu')
        nn.init.zeros_(self.in_proj.bias)

    def forward(self, rgb):  # rgb: [B, 3]
        scale = (0.2126*rgb[:,0:1] + 0.7152*rgb[:,1:2] + 0.0722*rgb[:,2:3]) # [B, 1]
        rgb_sin = torch.cat([self.pre_proj(rgb[:, 0:1]), self.pre_proj(rgb[:, 1:2]), self.pre_proj(rgb[:, 2:3])], dim=-1)
...
class SpectralMLP(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            x[0:x.shape[0]//2, 0:3] += torch.randn_like(x[0:x.shape[0]//2, 0:3]) * 0.1 / 256.0 # 
            x[0:x.shape[0]//2, 3:4] += torch.randn_like(x[0:x.shape[0]//2, 3:4]) * 0.1 / (780.0 - 360.0) # 
        ...
        return softclamp01_exp(result) * 1.01 # [B, 1]
...
FUNCTION_MODIFIER_INLINE void RGBEncode(const float* __restrict__ in3, float* __restrict__ outK) 
{
    ...
    const float I = (0.2126f*in3[0] + 0.7152f*in3[1] + 0.0722f*in3[2]);
    //const float inP3[3] = {sqrtf(in3[0]), sqrtf(in3[1]), sqrtf(in3[2])};        
    constexpr int RGB_SIN_WIDTH = (WIDTH / 6) * 2;
    float s[RGB_SIN_WIDTH * 3];
    // sincos: s = [sin(2piR), cos(2piR), sin(4piR), cos(4piR), ..., sin(2piG), cos(2piG), sin(4piG), cos(4piG), ..., sin(2piB), cos(2piB), sin(4piB), cos(4piB)]
    {
        //constexpr float TWO_PI = 6.28318530717958647692f;
        CUDA_UNROLL
        for (int k = 0; k < RGB_SIN_WIDTH/2; ++k)
        {
            const float phase = in3[0] * CUDART_PI_F * static_cast<float>(k + 1);
            sincosf(phase, &s[k], &s[k + RGB_SIN_WIDTH/2]);
        }
        CUDA_UNROLL
        for (int k = 0; k < RGB_SIN_WIDTH/2; ++k)
        {
            const float phase = in3[1] * CUDART_PI_F * static_cast<float>(k + 1);
            sincosf(phase, &s[k + RGB_SIN_WIDTH], &s[k + RGB_SIN_WIDTH/2 + RGB_SIN_WIDTH]);
        }
        CUDA_UNROLL
        for (int k = 0; k < RGB_SIN_WIDTH/2; ++k)
        {
            const float phase = in3[2] * CUDART_PI_F * static_cast<float>(k + 1);
    ...
    tensor::LinearRowMajor<RGB_SIN_WIDTH * 3, WIDTH, /*bBias=*/true, /*ReLU=*/true, /*Opt=*/true>(s, h, 0, 0, RGB_IN_W, RGB_IN_B);
""")
        for i in range(max(0, depth - 1)):
...
""")
        ...
        f.write(r"""
    float h[WIDTH];
    // sincos: h = [sin(2pix), cos(2pix), sin(4pix), cos(4pix), ...]
    {
        //constexpr float TWO_PI = 6.28318530717958647692f;
        CUDA_UNROLL
...
FUNCTION_MODIFIER_INLINE float SpectralInteract(const float* __restrict__ RGBKernel, const float* __restrict__ LambdaKernel) 
{
    ...
    // softclamp01_exp
    return minf((1.0f - expf(-fmaxf(acc, 0.0f))) * 1.01f, 1.0f);
}
""")
        testN = 100
...
def main(args):
    ...
    for epoch in range(args.epoch):
        for batch in train_loader:
            ...
            loss = torch.square(pred - val).mean() # + torch.abs(pred - val).mean()
            ...
            if iteration % args.eval_every == 0 or iteration == 1:
                ...
                if score_ema < best_score:
                    ...
                    export_spectral_mlp_to_cuda(model, best_export_path, width=width, depth=depth, kernel=kernel, affix_depth=affix_depth, lmin=lmin, lmax=lmax, mae_loss=mae_ema_max)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--val-length", type=int, default=1000000)
